...
public class Paddle : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Input.GetKey(KeyCode.W) && transform.position.y < maxValue) // checks if the key w is pushed and if the position isn't above the maxValue
            {
                transform.Translate(Vector3.up * speed * Time.deltaTime); // moves the paddle up
            }
            else if (Input.GetKey(KeyCode.S) && transform.position.y > -maxValue) // checks if the key s is pushed and if the position isn't below the maxValue
            {
